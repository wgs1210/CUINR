...
### Please note: 
After VCA, the file undergoes normalization, which causes its size to increase after storage(data_v2.zip). However, this is just part of the preprocessing before training, and this step can be done in memory. For the sake of demonstration, it is stored as a .mat file here. Therefore, it should not be considered as the file before compression. Alternatively, you can delete it after the program finishes.
