...
def load_yaml_as_dict(cfg_path):
    with open(cfg_path, encoding='utf-8') as yaml_file:
...
def loss_MSE(device, rec, gt):
    # loss_func = torch.nn.L1Loss(size_average=True).to(device)
    loss_func = torch.nn.MSELoss(size_average=True,reduce=True,reduction='mean').to(device)
    return [loss_func(rec[0], gt[0])]

def loss_L1(device, rec, gt):
    loss_func = torch.nn.L1Loss(size_average=True).to(device)
    # loss_func = torch.nn.MSELoss(size_average=True,reduce=True,reduction='mean').to(device)
    return [loss_func(rec[0], gt[0])]

def loss_abu(abu1, abu2):
    loss_func = torch.nn.MSELoss(size_average=True,reduce=True,reduction='mean')
    abuloss = loss_func(abu1[0], abu2[0])
    return abuloss
...
def loss_unmixing(device, abu):
    criterionSumToOne = SumToOneLoss().to(device)
    return criterionSumToOne(abu)
...
def get_ssim(recHSI, gtHSI):
    rec2 = recHSI.squeeze(0).detach().cpu().numpy()
    gt2 = gtHSI.squeeze(0).detach().cpu().numpy()

    matGt2 = gt2.transpose(1,2,0)
    matRec2 = rec2.transpose(1,2,0)

    ssim2 = structural_similarity(matGt2, matRec2, data_range=1.0)
    print(" ssim2 ", ssim2)
